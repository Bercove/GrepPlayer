	...
	function y(f, g) {
						...
						if (
							...
							(h.daysContainer.style.width = t + 'px'),
								(h.calendarContainer.style.width =
									t + (void 0 !== h.weekWrapper ? h.weekWrapper.offsetWidth : 0) + 'px'),
			...
			0 === h.selectedDates.length && ie(),
				void 0 !== e &&
					...
					(function (e) {
						...
						void 0 !== h.amPM &&
							e.target === h.amPM &&
							(h.amPM.textContent = h.l10n.amPM[o(h.amPM.textContent === h.l10n.amPM[0])]);
						...
						if (void 0 !== n.value && 2 === n.value.length) {
							var u = n === h.hourElement,
								f = n === h.minuteElement;
							s < a
								? ((s = r + s + o(!u) + (o(u) && o(!h.amPM))), f && j(void 0, -1, h.hourElement))
								: s > r &&
		...
		function O(e, t, n) {
			void 0 !== h.latestSelectedDateObj && h.latestSelectedDateObj.setHours(e % 24, t, n || 0, 0),
				h.hourElement &&
					h.minuteElement &&
					!h.isMobile &&
					((h.hourElement.value = i(h.config.time_24hr ? e : ((12 + e) % 12) + 12 * o(e % 12 == 0))),
		...
		function F(e, t, n, a) {
			return t instanceof Array
				...
				: (e.addEventListener(t, n, a),
				  void h._handlers.push({ element: e, event: t, handler: n, options: a }));
		}
		function N(e) {
			return function (t) {
				1 === t.which && e(t);
		...
		function H(e, t, n, a) {
			...
			return (
				...
				-1 === e.indexOf('hidden') &&
					0 === w(t, h.now) &&
					((h.todayDateElem = o), o.classList.add('today'), o.setAttribute('aria-current', 'date')),
				i
					? ((o.tabIndex = -1),
					  he(t) &&
							...
							'range' === h.config.mode &&
								(c(o, 'startRange', h.selectedDates[0] && 0 === w(t, h.selectedDates[0], !0)),
		...
		function W(e) {
			for (
				var t = e > 0 ? 0 : h.config.showMonths - 1, n = e > 0 ? h.config.showMonths : -1, a = t;
				a != n;
				a += e
			)
				for (
		...
		function R(e, t) {
			...
			return void 0 === a
				...
				? void (function (e, t) {
						for (
							var n = -1 === e.className.indexOf('Month') ? e.dateObj.getMonth() : h.currentMonth,
							...
							for (
								var r = h.daysContainer.children[o],
									l = n - h.currentMonth === o ? e.$i + t : t < 0 ? r.children.length - 1 : 0,
									c = r.children.length,
									d = l;
								d >= 0 && d < c && d != (t > 0 ? c : -1);
								d += i
		...
		function U() {
			...
			i.setAttribute('aria-label', h.l10n.yearAriaLabel),
				...
				h.config.maxDate &&
					...
					(i.disabled =
						!!h.config.minDate && h.config.minDate.getFullYear() === h.config.maxDate.getFullYear()));
			...
			if (h.isOpen && !h.config.inline) {
				var t = 'function' == typeof (r = e).composedPath ? r.composedPath()[0] : r.target,
					...
					a =
						...
						(e.path && e.path.indexOf && (~e.path.indexOf(h.input) || ~e.path.indexOf(h.altInput))),
					i =
						'blur' === e.type
				...
				i &&
					o &&
					(void 0 !== h.timeContainer && void 0 !== h.minuteElement && void 0 !== h.hourElement && T(),
					h.close(),
					'range' === h.config.mode && 1 === h.selectedDates.length && (h.clear(!1), h.redraw()));
			}
			var r;
		}
		function Q(e) {
			if (
				...
				(h.currentYear = t || h.currentYear),
					h.config.maxDate && h.currentYear === h.config.maxDate.getFullYear()
						? (h.currentMonth = Math.min(h.config.maxDate.getMonth(), h.currentMonth))
						: h.config.minDate &&
						  h.currentYear === h.config.minDate.getFullYear() &&
						  (h.currentMonth = Math.max(h.config.minDate.getMonth(), h.currentMonth)),
		...
		function X(e, t) {
			...
			if (
				(h.config.minDate && n && w(n, h.config.minDate, void 0 !== t ? t : !h.minDateHasTime) < 0) ||
				(h.config.maxDate && n && w(n, h.config.maxDate, void 0 !== t ? t : !h.maxDateHasTime) > 0)
			)
				return !1;
			if (0 === h.config.enable.length && 0 === h.config.disable.length) return !0;
			if (void 0 === n) return !1;
			for (
				var a = h.config.enable.length > 0, i = a ? h.config.enable : h.config.disable, o = 0, r = void 0;
				o < i.length;
				o++
			) {
			...
			if (13 === e.keyCode && t) {
				...
				switch (e.keyCode) {
					...
					case 40:
						...
						(h.daysContainer && void 0 !== e.target.$i) || e.target === h.input || e.target === h.altInput
							? e.ctrlKey
							...
							? Q(h.currentYear - l)
							: h.config.enableTime &&
							  (!o && h.hourElement && h.hourElement.focus(), T(e), h._debouncedChange());
						break;
					case 9:
						if (o) {
		...
		function ne(e) {
			if (
				...
				for (
					var t = e ? e.dateObj.getTime() : h.days.firstElementChild.dateObj.getTime(),
								...
								return u
									...
									: (['startRange', 'inRange', 'endRange', 'notAllowed'].forEach(function (e) {
									  ...
									  void (
											void 0 !== e &&
											(e.classList.add(
												t <= h.selectedDates[0].getTime() ? 'startRange' : 'endRange'
											),
											n < t && d === n
		...
		function re() {
			'object' != typeof h.config.locale &&
				...
				h.config.errorHandler(new Error('flatpickr: invalid locale ' + h.config.locale)),
				(h.l10n = e(
					{},
					E.l10ns.default,
					'object' == typeof h.config.locale
				...
				(p.K =
					...
					')'),
				void 0 === e({}, g, JSON.parse(JSON.stringify(f.dataset || {}))).time_24hr &&
		...
		function le(e) {
			if (void 0 !== h.calendarContainer) {
				...
				if (
					(c(h.calendarContainer, 'arrowTop', !s), c(h.calendarContainer, 'arrowBottom', s), !h.config.inline)
				) {
		...
		function se(e) {
			...
			if (void 0 !== t) {
				var n = t,
					...
					i =
						(a.getMonth() < h.currentMonth || a.getMonth() > h.currentMonth + h.config.showMonths - 1) &&
						'range' !== h.config.mode;
		...
		(h.parseDate = D({ config: h.config, l10n: h.l10n })),
			...
			(h.close = function () {
				(h.isOpen = !1),
					h.isMobile ||
						(void 0 !== h.calendarContainer && h.calendarContainer.classList.remove('open'),
			...
			(h.open = function (e, t) {
				...
				if (!0 === h.isMobile)
					return (
						...
						void 0 !== h.mobileInput && (h.mobileInput.focus(), h.mobileInput.click()),
						void ge('onOpen')
					);
		...
		function be(e) {
			...
			t || n
				...
				: e.target.classList.contains('arrowDown') && h.changeYear(h.currentYear - 1);
		}
		return (
			(function () {
				(h.element = h.input = f),
					...
					(function () {
						...
						for (var u = 0; u < h.config.plugins.length; u++) {
							...
							for (var p in m)
								t.indexOf(p) > -1
									? (h.config[p] = l(m[p]).map(y).concat(h.config[p]))
					...
					h.input
						? ((h.input._type = h.input.type),
						  ...
						  h.config.altInput &&
								...
								!h.config.static &&
									h.input.parentNode &&
									h.input.parentNode.insertBefore(h.altInput, h.input.nextSibling)),
					...
					(function () {
						...
						var e =
							...
							(('INPUT' !== h.input.nodeName && 'TEXTAREA' !== h.input.nodeName) ||
							!h.input.placeholder ||
						...
						e && fe(e, h.config.dateFormat),
							(h._initialDate =
								h.selectedDates.length > 0
									? h.selectedDates[0]
									: h.config.minDate && h.config.minDate.getTime() > h.now.getTime()
									? h.config.minDate
									: h.config.maxDate && h.config.maxDate.getTime() < h.now.getTime()
									? h.config.maxDate
									: h.now),
					...
					h.isMobile ||
						(function () {
							...
							if (
								...
								(h.calendarContainer.classList.add(h.config.inline ? 'inline' : 'static'),
								h.config.inline &&
									(!r && h.element.parentNode
										? h.element.parentNode.insertBefore(h.calendarContainer, h._input.nextSibling)
							...
							h.config.static ||
								h.config.inline ||
								(void 0 !== h.config.appendTo ? h.config.appendTo : window.document.body).appendChild(
									h.calendarContainer
								);
						})(),
					(function () {
						if (
							...
							!(function () {
								...
								try {
									h.input.parentNode &&
										h.input.parentNode.insertBefore(h.mobileInput, h.input.nextSibling);
								} catch (e) {}
							...
							(h._debouncedChange = r(Y, M)),
								...
								!0 === h.config.clickOpens &&
									(F(h._input, 'focus', h.open), F(h._input, 'mousedown', N(h.open))),
								void 0 !== h.daysContainer &&
					...
					(h.selectedDates.length || h.config.noCalendar) &&
						(h.config.enableTime &&
							I(h.config.noCalendar ? h.latestSelectedDateObj || h.config.minDate : void 0),
						we(!1)),
					x(),
