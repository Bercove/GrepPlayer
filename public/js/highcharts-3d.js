...
(function (r) {
	...
	A(r, 'parts-3d/Math.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		...
		b.perspective3D = function (b, l, v) {
			l = 0 < v && v < Number.POSITIVE_INFINITY ? v / (b.z + l.z + v) : 1;
		...
		b.perspective = function (l, x, v) {
			...
			return l.map(function (c) {
				...
				var m = (w ? c.x : c.y) - q.y;
				c = (c.z || 0) - q.z;
				e = { x: d * e - g * c, y: -k * g * e + a * m - d * k * c, z: a * g * e + k * m + a * d * c };
	...
	A(r, 'parts-3d/SVGRenderer.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		...
		l.prototype.face3d = function (a) {
			...
			f.attr = function (a) {
				if ('object' === typeof a && (q(a.enabled) || q(a.vertexes) || q(a.insidePlotArea))) {
					...
					var f = g(this.vertexes, p[e.chartIndex], this.insidePlotArea),
						n = e.toLinePath(f, !0);
					f = b.shapeArea(f);
		...
		l.prototype.polyhedron = function (a) {
			...
			f.attr = function (a, n, d, m) {
				if ('object' === typeof a && q(a.faces)) {
					...
					for (var b = 0; b < a.faces.length; b++)
						e.styledMode && delete a.faces[b].fill, f.faces[b].attr(a.faces[b], null, d, m);
					delete a.faces;
				}
				return c.prototype.attr.apply(this, arguments);
			};
			f.animate = function (a, n, d) {
				if (a && a.faces) {
					for (; f.faces.length > a.faces.length; ) f.faces.pop().destroy();
					for (; f.faces.length < a.faces.length; ) f.faces.push(e.face3d().add(f));
					for (var m = 0; m < a.faces.length; m++) f.faces[m].animate(a.faces[m], n, d);
					delete a.faces;
				}
				return c.prototype.animate.apply(this, arguments);
			};
		...
		var m = {
			initArgs: function (a) {
				...
				c.parts.forEach(function (a) {
					c[a] = f
						.path(e[a])
						.attr({ class: 'highcharts-3d-' + a, zIndex: n[a] || 0 })
						.add(c);
				});
		...
		var t = b.merge(m, {
			...
			attr: function (a, e, f, d) {
				...
				return a.shapeArgs || q(a.x)
					? this.singleSetterForParts('d', null, this.renderer[this.pathType + 'Path'](a.shapeArgs || a))
					: c.prototype.attr.call(this, a, void 0, f, d);
			},
			animate: function (a, e, f) {
				q(a.x) && q(a.y)
					? ((a = this.renderer[this.pathType + 'Path'](a)),
					  this.singleSetterForParts('d', null, a, 'animate', e, f),
		...
		b.SVGRenderer.prototype.arc3d = function (e) {
			...
			f.attr = function (a) {
				var e;
				'object' === typeof a && (e = m(a)) && (x(f.attribs, e), f.setPaths(f.attribs));
			...
			f.animate = function (a, e, d) {
				...
				if (p.duration) {
					...
					g &&
						(p.step = function (a, e) {
							function c(a) {
								return n[a] + (h(g[a], n[a]) - n[a]) * e.pos;
							}
							e.prop === t &&
								e.elem.setPaths(
									k(n, {
		...
		l.prototype.arc3dPath = function (a) {
			...
			b > E && m < E
				? ((g = g.concat(['L', c + v * w(D) + t, d + n * B(D) + z])),
				...
				: b > y - k &&
				  ...
				  (g = g.concat(['L', c + v * Math.cos(b), d + n * Math.sin(b)])),
	...
	A(r, 'parts-3d/Chart.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		...
		x(v, 'afterInit', function () {
			...
			this.is3d() &&
				(b.series || []).forEach(function (p) {
					'scatter' === (p.type || b.chart.type || b.chart.defaultSeriesType) && (p.type = 'scatter3d');
				});
		});
						...
						children: [
							{
								...
								children: [
									{ tagName: 'feFuncR', type: 'linear', slope: b.slope },
		...
		l(v.prototype, 'renderSeries', function (b) {
			...
			if (this.is3d()) for (; p--; ) (b = this.series[p]), b.translate(), b.render();
			else b.call(this);
		});
		x(v, 'afterDrawChartBox', function () {
			if (this.is3d()) {
				...
				this.frameShapes.bottom[q]({
					...
					faces: [
						{
							fill: b.color(a.bottom.color).brighten(0.1).get(),
							vertexes: [
								{ x: e, y: n, z: h },
								{ x: m, y: n, z: h },
								{ x: m, y: n, z: f },
				...
				this.frameShapes.right[q]({
					...
					faces: [
						{
							fill: b.color(a.right.color).brighten(0.1).get(),
							vertexes: [
								{ x: m, y: n, z: f },
								{ x: k, y: c, z: p },
								{ x: k, y: c, z: 0 },
				...
				this.frameShapes.front[q]({
					...
					faces: [
						{
							fill: b.color(a.front.color).brighten(0.1).get(),
							vertexes: [
								{ x: e, y: n, z: h },
								{ x: m, y: n, z: h },
								{ x: k, y: c, z: 0 },
		...
		v.prototype.retrieveStacks = function (b) {
			...
			this.series.forEach(function (g) {
				...
				a[d] ? a[d].series.push(g) : ((a[d] = { series: [g], position: k }), k++);
			});
			a.totalStacks = k + 1;
			return a;
		};
		v.prototype.get3dFrame = function () {
			...
			[].concat(h.xAxis, h.yAxis, h.zAxis).forEach(function (a) {
				a && (a.horiz ? (a.opposite ? (x = !0) : (r = !0)) : a.opposite ? (u = !0) : (y = !0));
			});
			...
			'auto' === p.axisLabelPosition
				? ((f = function (a, c) {
				  ...
				  (p = []),
				  f(a.left, a.front) && p.push({ y: (g + c) / 2, x: d, z: 0, xDir: { x: 1, y: 0, z: 0 } }),
				  f(a.left, a.back) && p.push({ y: (g + c) / 2, x: d, z: e, xDir: { x: 0, y: 0, z: -1 } }),
				  f(a.right, a.front) && p.push({ y: (g + c) / 2, x: k, z: 0, xDir: { x: 0, y: 0, z: 1 } }),
				  f(a.right, a.back) && p.push({ y: (g + c) / 2, x: k, z: e, xDir: { x: -1, y: 0, z: 0 } }),
				  (t = []),
				  f(a.bottom, a.front) && t.push({ x: (d + k) / 2, y: c, z: 0, xDir: { x: 1, y: 0, z: 0 } }),
				  f(a.bottom, a.back) && t.push({ x: (d + k) / 2, y: c, z: e, xDir: { x: -1, y: 0, z: 0 } }),
				  (n = []),
				  f(a.top, a.front) && n.push({ x: (d + k) / 2, y: g, z: 0, xDir: { x: 1, y: 0, z: 0 } }),
				  f(a.top, a.back) && n.push({ x: (d + k) / 2, y: g, z: e, xDir: { x: -1, y: 0, z: 0 } }),
				  (l = []),
				  f(a.bottom, a.left) && l.push({ z: (0 + e) / 2, y: c, x: d, xDir: { x: 0, y: 0, z: -1 } }),
				  f(a.bottom, a.right) && l.push({ z: (0 + e) / 2, y: c, x: k, xDir: { x: 0, y: 0, z: 1 } }),
				  (c = []),
				  f(a.top, a.left) && c.push({ z: (0 + e) / 2, y: g, x: d, xDir: { x: 0, y: 0, z: -1 } }),
				  f(a.top, a.right) && c.push({ z: (0 + e) / 2, y: g, x: k, xDir: { x: 0, y: 0, z: 1 } }),
				  (d = function (a, c, e) {
						if (0 === a.length) return null;
						if (1 === a.length) return a[0];
						for (var d = 0, f = w(a, h, !1), b = 1; b < f.length; b++)
							e * f[b][c] > e * f[d][c]
								? (d = b)
								: e * f[b][c] === e * f[d][c] && f[b].z < f[d].z && (d = b);
						return a[d];
				  }),
				  (a.axes = {
				...
				: (a.axes = {
						...
						x: {
							...
							bottom: { y: c, z: 0, xDir: { x: 1, y: 0, z: 0 } }
						},
						z: {
							top: { x: y ? k : d, y: g, xDir: y ? { x: 0, y: 0, z: 1 } : { x: 0, y: 0, z: -1 } },
							bottom: { x: y ? k : d, y: c, xDir: y ? { x: 0, y: 0, z: 1 } : { x: 0, y: 0, z: -1 } }
						}
				  });
			return a;
		};
	...
	A(r, 'parts-3d/Axis.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		function r(a, e, b) {
				...
				('flap' === f
					...
					? a.horiz
						? ((u = Math.cos(m)),
						  ...
						  (m = -Math.sin(m)),
						  (g = -u * Math.cos(g)),
						  (u = { x: k.y * g - k.z * m, y: k.z * f - k.x * g, z: k.x * m - k.y * f }),
						  (m = 1 / Math.sqrt(u.x * u.x + u.y * u.y + u.z * u.z)),
						  c && (m = -m),
						  (u = { x: m * u.x, y: m * u.y, z: m * u.z }))
						: (k = { x: Math.cos(g), y: 0, z: Math.sin(g) })
					: a.horiz
					? (u = { x: Math.sin(g) * Math.sin(m), y: Math.cos(m), z: -Math.cos(g) * Math.sin(m) })
					: (k = { x: Math.cos(g), y: 0, z: Math.sin(g) }));
			e.x += r * k.x + x * u.x;
			e.y += r * k.y + x * u.y;
			e.z += r * k.z + x * u.z;
			...
			b &&
				...
				(c.matrix = [a[1].x - a[0].x, a[1].y - a[0].y, a[2].x - a[0].x, a[2].y - a[0].y, c.x, c.y]),
		...
		l(h.prototype, 'getPlotLinePath', function (a) {
			...
			this.horiz
				? (this.isZAxis
						? (d.left.visible && b.push(c[0], c[2]), d.right.visible && b.push(c[1], c[3]))
						: (d.front.visible && b.push(c[0], c[2]), d.back.visible && b.push(c[1], c[3])),
		...
		v(h, 'drawCrosshair', function (a) {
			this.chart.is3d() &&
				'colorAxis' !== this.coll &&
				a.point &&
				(a.point.crosshairPos = this.isXAxis ? a.point.axisXpos : this.len - a.point.axisYpos);
		});
		...
		h.prototype.swapZ = function (a, d) {
			return this.isZAxis ? ((d = d ? 0 : this.chart.plotLeft), { x: d + a.z, y: a.y, z: a.x - d }) : a;
		};
				...
				a.series.forEach(function (c) {
					if (c.visible || !d.options.chart.ignoreHiddenSeries)
						(a.hasVisibleSeries = !0),
							(c = c.zData),
							c.length &&
								((a.dataMin = Math.min(q(a.dataMin, c[0]), Math.min.apply(null, c))),
								(a.dataMax = Math.max(q(a.dataMax, c[0]), Math.max.apply(null, c))));
				});
			}
		});
	...
	A(r, 'parts-3d/Series.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		...
		b.Series.prototype.translate3dPoints = function () {
			...
			b = u(v, b, !0);
			for (h = 0; h < this.data.length; h++)
				(w = this.data[h]), (l = b[h]), (w.plotX = l.x), (w.plotY = l.y), (w.plotZ = l.z);
		};
	});
	A(r, 'parts-3d/Column.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		function r(b) {
			var a = b.apply(this, [].slice.call(arguments, 1));
			this.chart.is3d &&
				this.chart.is3d() &&
				((a.stroke = this.options.edgeColor || a.fill), (a['stroke-width'] = x(this.options.edgeWidth, 1)));
			return a;
		}
		function u(b, a, d) {
					...
					(a
						...
						: (k.data.forEach(function (a) {
								null !== a.y &&
									...
									a.graphic && a.graphic.animate(a.shapeArgs, k.options.animation));
						  }),
						  this.drawDataLabels(),
						  (k.animate = null)));
		...
		l(y.column.prototype, 'plotGroup', function (b, a, d, k, g, c) {
			'dataLabelsGroup' !== a &&
				...
				c &&
					(this.chart.columnGroup || (this.chart.columnGroup = this.chart.renderer.g('columnGroup').add(c)),
		...
		l(y.column.prototype, 'setVisible', function (b, a) {
			...
			d.chart.is3d() &&
				d.data.forEach(function (b) {
					k = (b.visible = b.options.visible = a = 'undefined' === typeof a ? !x(d.visible, b.visible) : a)
						? 'visible'
						: 'hidden';
		...
		v(w, 'afterInit', function () {
			if (this.chart.is3d() && this.handle3dGrouping) {
				...
				if ('undefined' === typeof a || a) {
					...
					g = b.stack || 0;
					for (d = 0; d < a[g].series.length && a[g].series[d] !== this; d++);
		...
		y.columnrange &&
			...
			l(y.columnrange.prototype.pointClass.prototype, 'hasNewShapeType', q),
			(y.columnrange.prototype.plotGroup = y.column.prototype.plotGroup),
			(y.columnrange.prototype.setVisible = y.column.prototype.setVisible));
	...
	A(r, 'parts-3d/Pie.js', [r['parts/Globals.js'], r['parts/Utilities.js']], function (b, l) {
		...
		l(q.pie.prototype, 'translate', function (b) {
			...
			if (this.chart.is3d()) {
				...
				h.data.forEach(function (b) {
					...
					g = (g.end + g.start) / 2;
					b.slicedTranslation = {
						translateX: Math.round(Math.cos(g) * l.slicedOffset * Math.cos(p * u)),
						translateY: Math.round(Math.sin(g) * l.slicedOffset * Math.cos(p * u))
					};
				});
			}
		});
		...
		l(q.pie.prototype, 'pointAttribs', function (b, h, l) {
			...
			this.chart.is3d() &&
				...
				((b.stroke = l.edgeColor || h.color || this.color), (b['stroke-width'] = r(l.edgeWidth, 1)));
			return b;
		});
				...
				x &&
					...
					h
						? ((r.oldtranslateX = r.translateX),
						  ...
						  (h = { translateX: q[0], translateY: q[1], scaleX: 0.001, scaleY: 0.001 }),
						  r.attr(h),
						  p && ((p.attrSetters = r.attrSetters), p.attr(h)))
						: ((h = { translateX: r.oldtranslateX, translateY: r.oldtranslateY, scaleX: 1, scaleY: 1 }),
						  r.animate(h, l),
						  p && p.animate(h, l),
						  (this.animate = null)));
			} else b.apply(this, [].slice.call(arguments, 1));
	...
	A(r, 'parts-3d/Scatter.js', [r['parts/Globals.js']], function (b) {
		...
		r(
			'scatter3d',
			'scatter',
			{ tooltip: { pointFormat: 'x: <b>{point.x}</b><br/>y: <b>{point.y}</b><br/>z: <b>{point.z}</b><br/>' } },
			{
