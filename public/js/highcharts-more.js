...
(function (t) {
	...
	A(t, 'parts-more/RadialAxis.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
		...
		var p = {
			...
			getPlotBandPath: function (c, f, q) {
				...
				if ('polygon' === this.options.gridLineInterpolation)
					v = this.getPlotLinePath({ value: c }).concat(this.getPlotLinePath({ value: f, reverse: !0 }));
				else {
					...
					e &&
						...
						(v.xBounds = e > -Math.PI / 2 && e < Math.PI / 2 ? [a, this.chart.plotWidth] : [0, a]),
			...
			getCrosshairPosition: function (c, f, q) {
				...
				if (this.isCircular) {
					if (g(p))
						c.point &&
							((d = c.point.shapeArgs || {}), d.start && (p = this.translate(c.point.rectPlotY, !0)));
					else {
						var d = c.chartX || 0;
						var a = c.chartY || 0;
			...
			getPlotLinePath: function (c) {
				var f = this,
					...
					e = c.reverse,
					h = f.getPosition(a),
					n = f.pane.options.background ? f.pane.options.background[0] || f.pane.options.background : {},
					g = n.innerRadius || '0%',
					l = n.outerRadius || '100%';
				n = q[0] + p.plotLeft;
				q = q[1] + p.plotTop;
				...
				h = h.y;
				var u, m;
				c.isCrosshair && ((h = this.getCrosshairPosition(c, n, q)), (a = h[0]), (b = h[1]), (h = h[2]));
				if (f.isCircular) {
					e = 'string' === typeof g ? z(g, 1) : g / Math.sqrt(Math.pow(b - n, 2) + Math.pow(h - q, 2));
					c = 'string' === typeof l ? z(l, 1) : l / Math.sqrt(Math.pow(b - n, 2) + Math.pow(h - q, 2));
					var r = ['M', n + e * (b - n), q - e * (q - h), 'L', b - (1 - c) * (b - n), h + (1 - c) * (q - h)];
				} else
					(a = f.translate(a)),
		...
		e(l, 'init', function (c) {
			...
			if ('colorAxis' === e) this.isRadial = !1;
				...
				if (d) {
					if ((m(this, g ? n : p), (l = !h))) this.defaultPolarOptions = this.defaultRadialGaugeOptions;
				} else
					a &&
						(m(this, p),
		...
		e(d, 'afterGetLabelPosition', function (c) {
			...
			if (f.isRadial) {
				...
				'auto' === d.rotation
					? q.attr({ rotation: e })
					: null === a && (a = f.chart.renderer.fontMetrics(q.styles && q.styles.fontSize).b - p.height / 2);
				null === n &&
					(f.isCircular
						? (p.width > (f.len * f.tickInterval) / (f.max - f.min) && (h = 0),
						  (n = e > h && e < 180 - h ? 'left' : e > 180 + h && e < 360 - h ? 'right' : 'center'))
						: (n = 'center'),
					q.attr({ align: n }));
		...
		a(
			...
			},
			{
				...
				getGraphPath: function (a) {
					...
					a = l.call(this, a);
					f && (!0 === f && (f = 'left'), (n.step = { left: 'right', center: 'center', right: 'left' }[f]));
					e = l.call(this, e);
					d = l.call(this, d);
					n.step = f;
					n = [].concat(a, e);
	...
	A(t, 'parts-more/AreaSplineRangeSeries.js', [t['parts/Globals.js']], function (b) {
		var a = b.seriesType;
		a('areasplinerange', 'arearange', null, { getPointSpline: b.seriesTypes.spline.prototype.getPointSpline });
		('');
	});
	A(t, 'parts-more/ColumnRangeSeries.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
		var r = a.clamp,
			g = a.pick;
		...
		w(
			'columnrange',
			'arearange',
			m(a.column, a.arearange, { pointRange: null, marker: null, states: { hover: { halo: !1 } } }),
			{
				translate: function () {
					var a = this,
						l = a.yAxis,
			...
			},
			{
				...
				translate: function () {
					...
					this.points.forEach(function (d) {
						...
						if (g(q) || !1 === e.wrap)
							(q = g(q) ? (q / 180) * Math.PI : 0), (F = r(F, a.startAngleRad - q, a.endAngleRad + q));
						F = (180 * F) / Math.PI;
						d.shapeType = 'path';
						d.shapeArgs = {
							d: b.path || [
					...
					a.points.forEach(function (v) {
						...
						'undefined' !== typeof v.plotY &&
							...
							E &&
								...
								v.whiskers[k]({ d: ['M', n - C, h, 'L', n + C, h, 'M', n - C, d, 'L', n + C, d] })),
	...
	A(t, 'parts-more/WaterfallSeries.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
		var r = a.arrayMax,
			g = a.arrayMin,
			m = a.correctFloat,
			k = a.isNumber,
			...
			},
			{
				...
				translate: function () {
					...
					for (a = g.length; l < a; l++) {
						...
						y.height <= f && !m.isNull
							? ((y.height = f), (y.y -= q), (m.plotY = y.y), (m.minPointLengthOffset = 0 > m.y ? -q : q))
				...
				getCrispPath: function () {
					...
					for (l = 1; l < c; l++) {
						...
						if ((!b && w && 0 > m.y && !d) || (0 < m.y && d)) (w[2] += r.height), (w[5] += r.height);
						h = h.concat(w);
					}
					return h;
				},
	...
	A(t, 'parts-more/BubbleLegend.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
			...
			setOptions: function () {
				...
				a.forEach(function (q, p) {
					m ||
						...
						(b.fill = w(q.color, c.color, 1 !== n ? h(f.color).setOpacity(n).get('rgba') : f.color)),
			...
			hideOverlappingLabels: function () {
				...
				!this.options.labels.allowOverlap &&
					...
					c.labels.forEach(function (f, a) {
						f.newOpacity ? f.newOpacity !== f.oldOpacity && c.connectors[a].show() : c.connectors[a].hide();
					}));
			},
			getRanges: function () {
				...
				var h = e === b ? [{ value: b }] : [{ value: e }, { value: (e + b) / 2 }, { value: b, autoRanges: !0 }];
		...
		e.prototype.getLinesHeights = function () {
			...
			for (e = 0; e < f; e++)
				if (
					...
					a[e] === a[f - 1] || (a[e + 1] && a[e]._legendItemPos[1] !== a[e + 1]._legendItemPos[1]))
				) {
					c.push({ height: 0 });
					var b = c[c.length - 1];
		...
		a(l.prototype, 'drawChartBox', function (a, c, f) {
			...
			} else a.call(this, c, f), e && e.options.enabled && e.bubbleLegend && (e.render(), e.retranslateItems(e.getLinesHeights()));
		});
	});
			...
			this.series.forEach(function (f) {
				...
				!f.bubblePadding ||
					...
					h &&
						...
						(f = f.zData.filter(w)),
						f.length &&
							((x = t(d.zMin, m(g(f), !1 === d.displayNegative ? d.zThreshold : -Number.MAX_VALUE, x))),
	...
	A(t, 'parts-more/PackedBubbleSeries.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
		var r = a.clamp,
			g = a.defined,
			m = a.extend,
			k = a.extendClass,
				...
				translate: function () {
					...
					for (e = 0; e < n.length; e++)
						if (n[e][3] === c) {
							...
							var l = n[e][2];
							h ||
								((p.plotX = n[e][0] - a.plotLeft + a.diffX), (p.plotY = n[e][1] - a.plotTop + a.diffY));
						...
						if (1 < a.length)
							for (
								...
								(a[n][2] = a[n][2] || 1),
									...
									b(g, d[f][0])
										? (d.push([]), (h = 0), d[f + 1].push(c(d[f][e], d[f][0], a[n])), f++, (e = 0))
						...
						h &&
							...
							h.nodes.forEach(function (c) {
								a &&
									...
									0 > f &&
										(c.series.addPoint(b.merge(a.options, { plotX: a.plotX, plotY: a.plotY }), !1),
	...
	A(t, 'parts-more/Polar.js', [t['parts/Globals.js'], t['parts/Utilities.js']], function (b, a) {
		...
		e.toXY = function (a) {
			...
			this.kdByAngle
				? ((b = ((g / Math.PI) * 180 + e.pane.options.startAngle) % 360), 0 > b && (b += 360), (a.clientX = b))
				: (a.clientX = a.plotX);
		};
		z.spline &&
