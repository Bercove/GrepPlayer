...
(function (f) {
	...
	function Ka(a, b, c, d) {
		...
		if (null === e.nTr) {
			...
			for (k = a.aoColumns.length; p < k; p++) {
				...
				if (
					l ||
					!((c && !v.mRender && v.mData === p) || (f.isPlainObject(v.mData) && v.mData._ === p + '.display'))
				)
	...
	function ob(a) {
		...
		for (b = k.length; l < b; l++) {
			...
			a.oFeatures.bSort &&
				...
				!1 !== m.bSortable &&
					(p.attr('tabindex', a.iTabIndex).attr('aria-controls', a.sTableId), Pa(a, m.nTh, l)));
			...
			if (0 !== l.length)
				...
				(c = h.sZeroRecords),
					...
					(b[0] = f('<tr/>', { class: e ? d[0] : '' }).append(
						f('<td />', { valign: 'top', colSpan: V(a), class: a.oClasses.sRowEmpty }).html(c)
					)[0]);
	...
	function tb(a) {
		...
		var l = function () {
			...
			b != e.sSearch &&
				(ha(a, { sSearch: b, bRegex: e.bRegex, bSmart: e.bSmart, bCaseInsensitive: e.bCaseInsensitive }),
				(a._iDisplayStart = 0),
				S(a));
		};
	...
	function ma(a) {
		...
		if (a.scrollBarVis !== D && a.scrollBarVis !== n) (a.scrollBarVis = D), Z(a);
			...
			'' === c
				? ((ya.width = '100%'),
				  A &&
						(r.find('tbody').height() > k.offsetHeight || 'scroll' == p.css('overflow-y')) &&
			...
			for (q = 0; q < l.length; q++)
				(r = c[l[q]]),
					...
					r.sWidthOrig &&
						h &&
						f(m[q]).append(
							f('<div/>').css({ width: r.sWidthOrig, margin: 0, padding: 0, border: 0, height: 1 })
						);
			if (a.aoData.length)
				for (q = 0; q < l.length; q++)
		...
		for (var d = 0, e = b.length; d < e; d++) {
			...
			h.bSortable &&
				(0 < c.length && c[0].col == d
					? (f.setAttribute('aria-sort', 'asc' == c[0].dir ? 'ascending' : 'descending'),
					  (h = g[c[0].index + 1] || g[0]))
					: (h = g[0]),
	...
	function O(a, b, c, d) {
		...
		else if (((b = q.ext), (b = b.sErrMode || b.errMode), a && A(a, null, 'error', [a, d, c]), 'alert' == b))
			alert(c);
		else {
	...
	var q = function (a) {
			...
			this.each(function () {
				...
				if ('table' != this.nodeName.toLowerCase())
					...
					g.aLengthMenu &&
						!g.iDisplayLength &&
						(g.iDisplayLength = f.isArray(g.aLengthMenu[0]) ? g.aLengthMenu[0][0] : g.aLengthMenu[0]);
					g = $a(f.extend(!0, {}, p), g);
					M(
						r.oFeatures,
						g,
	...
	x.register = t = function (a, b) {
		if (f.isArray(a)) for (var c = 0, d = a.length; c < d; c++) x.register(a[c], b);
			...
			for (c = d.length; a < c; a++) {
				...
				a === c - 1
					? ((k.val = b),
					  (k.type = 'function' === typeof b ? 'function' : f.isPlainObject(b) ? 'object' : 'other'))
		...
		fc = function (a, b, c) {
			...
			return db(
				...
				function (b) {
					...
					if ('string' === typeof b && '#' === b.charAt(0) && ((e = a.aIds[b.replace(/^#/, '')]), e !== n))
						return [e.idx];
	...
	var gc = function (a, b, c, d) {
		...
		Yb = function (a, b) {
			...
			c.length &&
				a.length &&
				((a = c[0].aoData[a[0]]),
				a._details && ((a._detailsShow = b) ? a._details.insertAfter(a.nTr) : a._details.detach(), hc(c[0])));
		},
		hc = function (a) {
			var b = new x(a),
				c = a.aoData;
				...
				if (null === b || b === n || c) {
					...
					for (q = e.length; p < q; p++)
						for (k = e[p], u = 0; u < l; u++)
							(t = { row: k, column: u }),
								c
									? ((w = d[k]), b(t, F(a, k, u), w.anCells ? w.anCells[u] : null) && m.push(t))
									: m.push(t);
					return m;
				}
				...
				f(a.nTable).on('order.dt.DT', function (e, f, g, k) {
					a === f &&
						...
						b
							...
							.addClass(
								'asc' == k[e] ? d.sSortJUIAsc : 'desc' == k[e] ? d.sSortJUIDesc : c.sSortingClassJUI
							));
				});
			}
		}
